...
for d in range(2, 40):
    zyl = etree_html.xpath('//*[@id="root"]/div/div[3]/div[3]/p[' + str(d) + ']/span/span/span[3]/text()')
    for zyr in zyl:
        print(str(d) + zyr)
        cursor.execute("update content set cure='%s'  where id=%s" % (zyr, d))
for b in range(2,40):
...
for e in range(2, 40):
    swl = etree_html.xpath('//*[@id="root"]/div/div[3]/div[3]/p[' + str(e) + ']/span/span/span[4]/text()')
    for ysw in swl:
        print(str(e) + ysw)
        cursor.execute("update content set death='%s'  where id=%s" % (ysw, e))
for c in range(1,40):
